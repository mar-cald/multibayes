...
For a family of $m$ such tests, let $R$ denote the total number of rejections and $V$ the number of false rejections (true nulls that are rejected). The target error rate is the False Discovery Rate (FDR),
$$
\mathrm{FDR} = \mathbb{E}\!\left[\frac{V}{\max(R, 1)}\right],
$$
where $\max(R, 1)$ sets the ratio to zero when no hypothesis is rejected [@Benjamini1995]. The FDR decomposes as the product of the positive False Discovery Rate [pFDR, @Storey2003] and the probability of making any rejection, $\mathrm{FDR} = \mathrm{pFDR} \times \Pr(R > 0)$. For a single test with prior null probability $\pi_0$, the pFDR can be written as
$$
\mathrm{pFDR} = \frac{\pi_0\,\alpha}{\pi_0\,\alpha + (1 - \pi_0)(1 - \beta)},
$$
where $\alpha$ and $1 - \beta$ are the per-test Type I error rate and power [@Storey2003; @berger2023]. The pFDR is increasing in $\alpha$, so lowering the per-test error rate lowers the pFDR and, with it, the FDR.

The proposed correction acts on exactly these two quantities: it fixes $\pi_0$ through the researcher's prior and implicitly reduces $\alpha$ by raising the decision threshold. To see this, we can write the adjusted rule $pd_{\mathrm{adj},i} > c$ in odds form and substitute the adjusted odds,
$$
\frac{pd_{\mathrm{adj},i}}{1 - pd_{\mathrm{adj},i}} = \frac{pd_i}{1 - pd_i} \cdot \frac{1 - \pi_0}{\pi_0}.
$$
The rule reduces to a condition on the unadjusted odds,
$$
\frac{pd_i}{1 - pd_i} > \frac{c}{1 - c} \cdot \frac{\pi_0}{1 - \pi_0},
$$
which, on the probability scale, is a stricter threshold on the raw $pd_i$,
$$
pd_i > c^*, \qquad c^* = \frac{c\,\pi_0}{(1 - c)(1 - \pi_0) + c\,\pi_0},
$$
with $c^* > c$ whenever $\pi_0 > 0.5$. A stricter threshold lowers the per-test probability of rejecting a true null. This rate equals $\alpha^* = 2(1 - c^*)$ when the null distribution of $pd$ is approximately uniform on $[0.5, 1]$ (diffuse within-model prior or large $n$), and is smaller under an informative prior, which shrinks the posterior toward the null. Since $\alpha^* < \alpha$, the adjustment reduces the pFDR, and hence the FDR, relative to the unadjusted rule.^[The adjustment targets the FDR, not the FWER (the probability of at least one false positive). A test produces a false positive only when it is both null and rejected, with probability $\pi_0\,\alpha^*$, so across $m$ independent tests $\mathrm{FWER} = 1 - (1 - \pi_0\,\alpha^*)^{m}$. Controlling it at a target $\alpha$ gives $\alpha^* = \frac{1 - (1 - \alpha)^{1/m}}{\pi_0}$. Because the two-sided rule $pd > c$ has per-test Type I rate $\alpha^* = 2(1 - c)$ (the convention $c = 1 - \alpha/2$ used throughout), inverting this relation turns the family-wise correction into a single cutoff on the raw $pd$, $c_{\mathrm{fwer}} = 1 - \alpha^*/2 = 1 - \tfrac{1 - (1 - \alpha)^{1/m}}{2\,\pi_0}$. This uses the same identity $\alpha^* = 2(1-c)$, so it likewise assumes a diffuse prior or large $n$ and independent tests; an informative prior lowers the per-test rate and makes $c_{\mathrm{fwer}}$ conservative, controlling the FWER at or below the target.] 
