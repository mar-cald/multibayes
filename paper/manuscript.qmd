...
For a family of $m$ such tests, let $R$ denote the total number of rejections and $V$ the number of false rejections (true nulls that are rejected). The quantity we want to consider is the False Discovery Rate (FDR) [@Benjamini1995],
$$
\mathrm{FDR} = \mathbb{E}\!\left[\frac{V}{\max(R, 1)}\right],
$$
where the $\max(R, 1)$ in the denominator sets the ratio to zero when no hypothesis is rejected. The FDR can be decomposed as the product of the positive False Discovery Rate (pFDR) [@Storey2003],
$$
...
If each hypothesis is null with prior probability $\pi_0$ and non-null with probability $1 - \pi_0$, the pFDR is given by
$$
\mathrm{pFDR} = \frac{\pi_0\, \alpha}{\pi_0\, \alpha + (1 - \pi_0)(1 - \beta)},
$$
where $\alpha$ and $\beta$ are the per-test Type I and Type II error probabilities [@Storey2003; @berger2023]. This expression shows that the pFDR is governed by the null probability $\pi_0$ and the per-test error rates $\alpha$ and $\beta$. 
