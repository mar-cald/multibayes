...
The global prior probability that **all** hypotheses are null, *p*, is decomposed into a per-hypothesis prior:
...
Because the prior is conservative ($P(H_0) > P(H_1)$), the adjustment always 
...
| `p` | Global null probability supplied by the user |
| `m` | Family size used (*m* or $m_\text{eff}$) |
| `pm` | Per-hypothesis null prior $p^{1/m}$ |

When a `pd` vector is supplied directly, `mean.est`, `null.value`, and `direction` are not returned.

### Choosing `p`, `null.value`, and `direction`

- **`p`** encodes your prior belief that **all** tested hypotheses are simultaneously null. 
